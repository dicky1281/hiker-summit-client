...
import { PutHandler, PutImage } from "../../apiCalls/apiCalls";
import Modal from "./PopUp";
import {lastName, name ,phone,username, verify, updateProfilePicture } from '../../features/userSlice'
...
const DashboardAccount = () => {
    ...
    const [formData, setFormData] = useState({
      ...
      const handleSend = async (e) => {
        ...
        try {
          const { data:uploadResponse } = await privateInstance.put(`/api/v1/users/profile_picture/${user._id}`,data)
      ...
      const handleVerify = async () =>{
        try {
          ...
          alert('Silahkan Lakukan Konfirmasi Melalui Email Yang Telah Kirimkan, dan coba untuk login kembali')
  ...
  return (
    <div className="account">
          ...
          <Col lg={5} md={12}>
            <div className="profile-detail">
              ...
              <div className="detail">
                ...
                <p>{user.verified === false ? (<p style={{ color:"blue", textDecoration:"underline", cursor:"pointer" }} onClick={handleVerify}>Tidak Terverifikasi</p>) : "Terverifikasi"}</p>
              </div>
              ...
              <Button variant="warning" onClick={handleShow2}>Edit Profile</Button>
              <Button variant="warning" onClick={handleShow3}>Ganti Password</Button>
            </div>
          </Col>
          <Col lg={{ span: 5, offset: 2 }} className='profile-image'>
            <div className="image">
              <img src={`/api/v1/assets?bucket=${user.image_assets.bucket}&key=${user.image_assets.assets_key}`} alt="" />
              <Form.Group controlId="formFile" className="pt-4">
                <Form.Control type="file" onChange={handleSend}/>
              </Form.Group>
            </div>
          </Col>
        </Row>
      ...
      password={confirm.password}
        ...
        er={error ? (<h5 className="pt-3" style={{ color:"red" }}>Password Tidak Sama</h5>): ""}>
      </Modal>
    </div>
  );
};
