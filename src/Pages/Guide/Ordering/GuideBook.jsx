...
const GuideBook = () => {
  ...
  const handleTrack = () => {
    ...
    setBook((prev) => ({
      ...prev,
      track_route:  [selected[0]?.location?.track.filter((e) => e.track_name === Object.values(combine)[0])[0]],
    }));
  }
  ...
  const submitHandler = async(e) =>{
    ...
    try {
      ...
      const response =  await privateInstance.post(`/api/v1/bookings/users/${user._id}/destinations/${selected[0]._id}`,book)
  ...
  <Form.Group className="mb-3">
      ...
      <Form.Select id="disabledSelect" required onChange={(event)=> 
        {setBook((prev)=>({...prev,destination_id:event.target.value})) 
    ...
    <Form.Group className="mb-3">
      ...
      <Form.Select id="track_name" required disabled={selected.length === 0 ? true: false} onChange={(event) => {setCombine((prev) => ({
              ...prev,
              [event.target.id]: event.target.value
            })); setTrack(selected[0]?.location?.track.filter((e) => e.track_name === event.target.value))}}>
      <option>---</option>
  ...
  <Row className='mb-3'>
    ...
    <Form.Label htmlFor="disabledSelect">Naik</Form.Label>
      <Form.Control type='date' placeholder="First name" required onChange={(event) => setBook ((prevState)=>({
                  ...prevState,
                  date:{
                    ...prevState.date,
    ...
    <Form.Label htmlFor="disabledSelect">Turun</Form.Label>
      <Form.Control type='date' placeholder="Last name" required  onChange={(event) => setBook ((prevState)=>({
                  ...prevState,
                  date:{
                    ...prevState.date,
  ...
  <Form.Label htmlFor="disabledSelect">Jumlah Pendaki</Form.Label>
      <Form.Control type='number' min="1" max="5" placeholder="jumlah pendaki" className='mb-3' onChange={(event) => setBook({...book, hiker_count: event.target.value})}/>
      <Form.Group className="mb-3" controlId="exampleForm.ControlTextarea1" />
    <Form.Label>Tinggalkan Pesan.</Form.Label>
    <Form.Control as="textarea" className='mb-3' required rows={3} onChange={(event) => setBook({...book, note: event.target.value})}/>

    <Form.Group className="mb-3" id="formGridCheckbox">
    <Form.Check type="checkbox" required label="Saya setuju bahwa pemandu ditentukan oleh pihak HikerSummit yang telah disiapkan sebaik-mungkin." />
  </Form.Group>
    <Button type="submit" variant='success' className='mb-3'>Ajukan</Button>
  </fieldset>
</Form>
<Modal
        show={show}
      ...
      >
          ...
          <h3 style={{ padding:"10px",textAlign:"center" }}>Informasi Detail</h3>
      
        <Modal.Body>
          <div className="form-content">
            <div className="konten d-flex" style={{ justifyContent:"space-between", padding:"0px 20px 0px 20px" }}>
            <h6>Gunung Tujuan</h6>
            <h6 style={{ fontWeight:"700" }}>Gunung {convert?.[0]?.title}</h6>
            </div>
            <div className="konten d-flex" style={{ justifyContent:"space-between", padding:"0px 20px 0px 20px" }}>
            <h6>Jalur Pendakian</h6>
            <h6 style={{ fontWeight:"700" }}>{book?.track_route?.[0]?.track_name}</h6>
            </div>
            <div className="konten d-flex" style={{ justifyContent:"space-between", padding:"0px 20px 0px 20px" }}>
            <h6>Tanggal Pendakian</h6>
            <h6 style={{ fontWeight:"700" }}>{book?.date?.departure} - {book?.date?.arrival}</h6>
            </div>
            <div className="konten d-flex" style={{ justifyContent:"space-between", padding:"0px 20px 0px 20px" }}>
            <h6>Jumlah Pendaki</h6>
            <h6 style={{ fontWeight:"700" }}>{book?.hiker_count} Orang</h6>
            </div>
            <div className="konten d-flex" style={{ justifyContent:"space-between", padding:"0px 20px 0px 20px",flexDirection:"column" }}>
              <h6>Catatan:</h6>
              <h6 style={{ opacity:"0.7" }}>{book?.note}</h6>
            </div>
            <div className="konten d-flex" style={{ justifyContent:"space-between", padding:"0px 20px 0px 20px" }}>
            <h6>Total Harga</h6>
            <h6 style={{ fontWeight:"700" }}><CurrencyFormat value={price * d} decimalSeparator={','} displayType={'text'} thousandSeparator={'.'} prefix={'Rp.'} /></h6>
            </div>
        ...
        <Modal.Footer>
          <Button style={{opacity:"0.8"}} variant="secondary" onClick={handleClose}>Kembali
           <i className="mdi mdi-bell-minus-outline:"></i>
          </Button>
          <Button variant="success" type='submit' onClick={submitHandler}>Ajukan</Button>
        </Modal.Footer>
      </Modal>
